...
run: python src/generate_lla_plus.py
...
def register_section(secheading, all_phrase, sec_value, phrase_cnt, test_name):
    ...
    if sec_key == 'someone who hates you and wants to harm you%PHRASE0%enemy' and \
        sec_value.startswith("%PHRASE0%enemy%EXAMPLE0%The detective wanted"):
...
def parse_exponent(exp):
    ...
    """
    assert exp.get("class") == ["Exponent"]

    kids = exp.find_all("span", recursive=False)
    # 至少要有前 6 个
    #assert len(kids) >= 6, f"Exponent too short: {[k.get('class') for k in kids]}"
    for idx, kid in enumerate(kids):
        #assert kid.get("class") in allowed_exponent_child
        if kid.get("class") == ["EXP"]:
            None
        elif kid.get("class") == ["PRON"]:
            None
        elif kid.get("class") == ["LABEL"]:
            None
        elif kid.get("class") == ["GRAM"]:
            None
        elif kid.get("class") == ["DEF"]:
            None
        elif kid.get("class") == ["Exas"]:
            None
        elif kid.get("class") == ["RunOn"]:
            None
        elif kid.get("class") == ["ACTIVATOR"]:
            parse_activator(kid)
        else:
            assert False, f"test_name={test_name}"
...
def parse_section(section):
    ...
    # 固定前 4 个
    assert kids[0].get("class") == ["sk_title"]
    assert kids[1].get("class") == ["SECDEF"]
    assert kids[2].get("class") == ["word_table"]
    assert kids[3].get("class") == ["Exponent"]
    parse_exponent(kids[3])

    assert len(kids) == 4 or len(kids) == 5, f"test_name={test_name}, Section children too few: {[k.get('class') for k in kids]}"

    if len(kids) == 5:
        if kids[4].get("class") == ["ACTIVATOR"]:
            parse_activator(kids[4])
        else:
            assert kids[4].get("class") == ["addtion"], f"test_name={test_name}, Section children too few: {[k.get('class') for k in kids]}"
...
def do_the_job():
    ...
    with open(filename, encoding="utf8") as ifile:
        for line in ifile:
            ...
            # 找到 LLA header
            concept = soup.find_all("span",class_="Concept")
            assert len(concept) == 1, f"test_name={test_name}"

            concept_spans = concept[0].find_all("span", recursive=False)
            assert len(concept_spans) == 2, f"test_name={test_name}" # todo, fix the 'about' here.
            assert concept_spans[0].get("class") == ["HWD"], f"test_name={test_name}"
            if concept_spans[1].get("class") == ["addtion"]:
                # 这个body没有, 不用处理了.
                continue
            assert concept_spans[1].get("class") == ["ACTIVATOR"], f"test_name={test_name}"
            activator = concept_spans[1]

            parse_activator(activator)

            activator_spans = concept_spans[1].find_all("span", recursive=False)
            assert activator_spans[0].get("class") == ["Section"], f"test_name={test_name}"

            # for lla_idx, lla_header in enumerate(concept):
            ...
            #                     tmp = f"%EXAMPLE{example_idx_in_cur_subphrase}%{example}"
            ...
            #                     tmp = f"%THESPROPFORM{subphrase_idx_in_phrase}%{thespropform}"
            #                     if log_info: print(tmp)
            #                     seccontent += tmp
    ...
    #         sechead = section_map[sec_key]["sec_heading"] # todo, rename for consistency
    #         seccontent = section_map[sec_key]["content"]
    #         # 你可以自行决定中间的分隔方式
    ...
    print(f"identical section number = {len(section_map)}")
    print(f"by average, each section has been seen %.2f times in the mdx." % (float(total_sec_cnt)/len(section_map)))

    print(f"\ntotal phrase num in identical sections = {total_phrase_num_in_identical_sections}")
    print(f"by average, each section has phrase num = {total_phrase_num_in_identical_sections} / {len(section_map)} = %.2f" % \
          (float(total_phrase_num_in_identical_sections)/len(section_map)))
...
if __name__ == '__main__':
    ...
    dict_file = open("data/Longman Language Activator 2nd Ed.dict", "rb")
    idx_file = open("data/Longman_Language_Activator_2nd_Ed.idx", "rb")
    ...
    with open("stardict_output.txt", "w", encoding="utf-8") as f:
        #f.write(text)

        while True:
            try:
                word = read_cstring(idx_file)
                offset = struct.unpack(">I", idx_file.read(4))[0]
                size = struct.unpack(">I", idx_file.read(4))[0]
                ...
                f.write('\n\n')
                # print(f"===== {word} =====")
                # #print(text[:600])   # 只打印前 300 字，防止刷屏
                # print(text)
                # print()
